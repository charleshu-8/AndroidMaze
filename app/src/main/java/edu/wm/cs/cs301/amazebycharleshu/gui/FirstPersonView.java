...
import edu.wm.cs.cs301.amazebycharleshu.generation.BSPBranch;
import edu.wm.cs.cs301.amazebycharleshu.generation.BSPLeaf;
import edu.wm.cs.cs301.amazebycharleshu.generation.BSPNode;
import edu.wm.cs.cs301.amazebycharleshu.generation.Floorplan;
import edu.wm.cs.cs301.amazebycharleshu.generation.Wall;
...
public class FirstPersonView {
	...
	private final String TAG = "FirstPersonView";
	...
	public void draw(MazePanel panel, int x, int y, int walkStep, int ang, float percentToExit) {
        ...
        if (null == panel) {
            Log.v(TAG, "Can't get graphics object to draw on, mitigate this by skipping draw operation") ;
            return;
        }
        gc = panel;
